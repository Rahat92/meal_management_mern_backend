...
exports.getAProductCategory = async (req, res) => {
  try {
    const { id } = req.params;
    const productCategory = await ProductCategory.findById(id).populate('productTags');
    if (!productCategory) {
...
//         }
//       },
//       { $unwind: { path: "$shoppingComments.comment.tags", preserveNullAndEmptyArrays: true } },
